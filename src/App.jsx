...
const PROMPTS = {
  semiconductor: '최신 반도체 뉴스 10개를 찾아줘. TSMC, 삼성, NVIDIA, Intel, AMD, SK하이닉스 관련 뉴스 위주로. TechCrunch, The Verge, Tom\'s Hardware, EE Times 같은 테크 전문지 기사 위주로.',
  ai: '최신 AI 인공지능 뉴스 10개를 찾아줘. 주요 AI 회사들의 새로운 모델, 연구, 비즈니스 뉴스 위주로.',
  llm: '최신 LLM 대형언어모델 뉴스 10개를 찾아줘. GPT, Claude, Gemini, Llama 등 주요 모델 관련 뉴스 위주로.',
}
...
async function fetchNewsForTopic(topic, apiKey) {
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      system: '너는 테크 뉴스 큐레이터야. 웹 검색으로 최신 뉴스를 찾고 반드시 아래 형식의 JSON 배열만 반환해. 다른 말은 절대 하지마.\n\n[{"title":"제목","summary":"2-3문장 요약","source":"출처","url":"링크","publishedAt":"날짜"}]\n\n반드시 JSON 배열만, 마크다운 없이 반환.',
      messages: [{ role: 'user', content: PROMPTS[topic.id] + '\n\n반드시 JSON 배열 형식으로만 답해줘. 10개 항목.' }]
    })
  })
  if (!res.ok) {
  ...
  if (!Array.isArray(items) || items.length === 0) throw new Error('뉴스 데이터가 없습니다.')
  return items
}
