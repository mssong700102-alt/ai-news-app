...
const TOPICS = [
  { id: 'semiconductor', label: '반도체', query: 'semiconductor chip TSMC Samsung NVIDIA Intel AMD news site:techcrunch.com OR site:theverge.com OR site:anandtech.com OR site:tomshardware.com OR site:eetimes.com', color: '#FF6B35' },
  { id: 'ai', label: 'AI', query: 'AI artificial intelligence LLM GPU news site:techcrunch.com OR site:theverge.com OR site:wired.com OR site:venturebeat.com', color: '#00D4FF' },
  { id: 'llm', label: 'LLM', query: 'LLM large language model GPT Claude Gemini Llama news site:techcrunch.com OR site:theverge.com OR site:venturebeat.com', color: '#A855F7' },
]

const NEWS_KEY = 'ainews_data_v3'
...
async function fetchNewsForTopic(topic, apiKey) {
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      system: `You are a tech news curator specializing in ${topic.label}. Search multiple times to find at least 10 recent news items from top tech publications like TechCrunch, The Verge, AnandTech, Tom's Hardware, EE Times, VentureBeat, Wired, Reuters, Bloomberg. Return ONLY a JSON array with exactly 10 items. Each item: title (Korean preferred), summary (2-3 sentences in Korean), source (string), url (string), publishedAt (string). Return ONLY valid JSON array. No markdown, no explanation.`,
      messages: [{ role: 'user', content: `Search for the 10 most important ${topic.label} news from the last 48 hours. Focus on: ${topic.query}. Must return exactly 10 news items as JSON array with summaries in Korean.` }]
    })
  })
